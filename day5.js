// // // let name = "Manish Basnet"
// // // let age = 22 
// // // let address = "Dharan"

// // let me = Object.freeze({
// //     name : "Manish Basnet", 
// //     age : 22, 
// //     address : "Dharan"
// // })


// // // console.log(me.age)
// // // console.log(me.address)
// // // console.log(me["age"])
// // // console.log(me["address"])

// // // me.college = "Itahari Namuna College"
// // // me.haha = "hehehehe"
// // console.log(Object.keys(me))
// // console.log(Object.values(me))

// let myInfo = {
//     name : "Manish Basnet", 
//     age : 22, 
//     address : "Dharan"
// }
// // let age = myInfo.age
// // let address = myInfo.address
// // let name = myInfo.name

// // destructure 
// // let {name,age} = myInfo

// // console.log(age,name)

// // conditional statements
// let isRaining = true 

// // if(isRaining){
// //     console.log("Carry Umbrella")
// // }else{
// //     console.log("Dont carry umbrella")
// // }

// // ternary operator condition ? "if true" : "if false"
// // isRaining ? console.log("Carry Umbrella") : console.log("Dont carry umbrella")

// // short-circuit condition && "if left ko condition true vayo vane "
// // isRaining && console.log("Carry Umbrella")
// // age > 21, You can vote, else You cannot vote
// let age = 21
// if(age == 21){
//     console.log("You can vote")
// }else{
//     console.log("You cannot vote")
// }

// let temperature = 25
// if(temperature > 30){
//     console.log("Dherae garmi xa")
// }else if(temperature > 20){
//     console.log("Normal thik thak xa")
// }else{
//     console.log("Nikai jado xa bahira!!")
// }


let myInfo = {
    name : "Manish", 
    age : "22", 
    address : "Dharan"
}

// if(myInfo.age === 22 || myInfo.address == "Dharan" && myInfo.name == "Manish" ){
//     console.log("You can vote in dharan")
// }else{
//     console.log("You cannot vote")
// }

// myInfo.age === 22 || myInfo.address == "Dharan" || myInfo.name == "Manish" ? console.log("You can vote in dharan") : console.log("You cannot vote")

// myInfo.age === 22 && myInfo.address == "Dharan" && console.log("You can vote in dharan") 

loops 
functions 